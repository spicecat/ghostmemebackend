...
const register = async (req, res, next) => {
    ...
    try {
        ...
        const response = await superagent.post(url, nullifyUndefined({ name, email, phone, username, imageBase64: null })).set('key', apiKey)
...
const login = async (req, res, next) => {
    try {
        ...
        if (err.status === 555) setTimeout(async () => { await login(req, res, next) }, 1500)
        else res.status(400).send({ error: err.response.body.error })
    }
}
